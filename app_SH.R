...
########API key for accessing openFDA adverse event database##########
dbuijs_api_key <- "QPV3z0OxVhzLfp5EGZTLT6QZGcTRrtXpYebmAs4n"

##########Query to fetch specific results from database##########

#Fetch & count generic names of drugs with adverse events reported
...
server <- function(input, output) {
  ...
  #############Re-format adverse event onset age for adrplot###############
  ages <- reactive({
    data <- faers_query()
    #select patient onset unit of the YEAR & count number of report 
  ...
  output$reporterplot <- renderGvis({
   ...
   # for funtion left_join, if by=null it will just use the common cols; select label (e.g.Physician) & number of reports for that label 
    ...
    unknown <- data$openfda_query %>%
      ...
      .$results %>%
      .$total
    #combine unknow qualification to reporter_results to be incorporated into the pir chart
  ...
  ################Create Serious Reason Reports pie chart##################
  ...
  output$agegroupplot <- renderGvis({
    ...
    age_groups <- mutate(ages,
                         age_group = NA,
                         age_group = ifelse(term <= 28/365, "Neonate", age_group), #neonate = less than 28 days 
                         age_group = ifelse(term <= 2 & term > 28/365, "Infant", age_group), # infant = greater that 28 days & less that 2 yrs
                         age_group = ifelse(term > 2 & term < 12, "Child", age_group), # child = greater than 2 yrs & less than 12 yrs 
                         age_group = ifelse(term >= 12 & term < 18, "Adolescent", age_group), # adolescent = greater than 12 yrs & less than 18 yrs
                         age_group = ifelse(term >= 18 & term < 65, "Adult", age_group), # adult = greater than 18 yrs & less than 65 yrs
                         age_group = ifelse(term >= 65, "Elderly", age_group), # elderly = greater than 65 yrs
