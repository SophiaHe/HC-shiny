...
topingd_func <- function(n){
  ...
  cv_report_drug_dm <- cv_report_drug %>%
    select(REPORT_ID, DRUG_PRODUCT_ID)
  
  cv_drug_product_ingredients_dm <- cv_drug_product_ingredients %>%
  ...
  topingd <-  dplyr::summarise(group_by(ingd_master, ACTIVE_INGREDIENT_NAME),count=n_distinct(REPORT_ID)) %>% as.data.table()
...
topdrug_func <- function(n){
  ...
  topdrugs <- dplyr::summarise(group_by(drugs_master, DRUGNAME),count=n_distinct(REPORT_ID)) %>% as.data.table()
...
toprxn_func <- function(n){
  ...
  toprxn <- dplyr::summarise(group_by(rxn_master, PT_NAME_ENG),count=n_distinct(REPORT_ID)) %>% as.data.table()
